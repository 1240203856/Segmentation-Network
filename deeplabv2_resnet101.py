...
class ASPP(nn.Module):
    def __init__(self,num_classes,block=Bottleneck):
...
class Deeplabv2(nn.Module):
    def __init__(self,num_class,pretrain=True,_print=True):
        super(Deeplabv2, self).__init__()
        self.backbone=resnet101(pretrained=pretrain)
        self.ASPP=ASPP(num_classes=num_class)
        if _print:
    ...
    def forward(self,x):
        ...
        x=self.backbone(x)
        x=self.ASPP(x)
        x = nn.UpsamplingBilinear2d(size)(x)    
        return x
